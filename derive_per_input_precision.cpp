...
struct Env{
 ...
 arb_t aa,ss,ref,tmpa;
 arf_t av,p,base,third,coef;
 Env():c(MAXT){
   arb_poly_t s,z;arb_poly_init(s);arb_poly_init(z);arb_t one,p3,tmp;arb_init(one);arb_init(p3);arb_init(tmp);arb_init(aa);arb_init(ss);arb_init(ref);arb_init(tmpa);
   arf_init(av);arf_init(p);arf_init(base);arf_init(third);arf_init(coef);
   arb_one(one);arb_poly_set_coeff_si(s,0,-2);arb_poly_set_coeff_si(s,1,1);
   std::fprintf(stderr,"[setup] 8192-bit coefficient manufacture\n");arb_poly_zeta_series(z,s,one,0,MAXT+1,COEFF_BITS);arb_set_ui(p3,9);
   for(int m=0;m<MAXT;m++){arf_init(c.data()+m);arb_poly_get_coeff_arb(tmp,z,m+1);arb_t inv;arb_init(inv);arb_inv(inv,p3,COEFF_BITS);arb_add(tmp,tmp,inv,COEFF_BITS);arb_clear(inv);arf_set_round(c.data()+m,arb_midref(tmp),COEFF_BITS,ARF_RND_NEAR);arb_mul_ui(p3,p3,3,COEFF_BITS);}
   arb_clear(tmp);arb_clear(p3);arb_clear(one);arb_poly_clear(z);arb_poly_clear(s);
 }
 ~Env(){for(auto &x:c)arf_clear(&x);arf_clear(coef);arf_clear(third);arf_clear(base);arf_clear(p);arf_clear(av);arb_clear(tmpa);arb_clear(ref);arb_clear(ss);arb_clear(aa);}
...
static std::vector<uint16_t> build_lut(Env&e){std::vector<uint16_t>L(BINS,53);const double f[]={0.015625,0.0625,0.125,0.25,0.375,0.5,0.625,0.75,0.875,0.9375,0.984375};for(int b=0;b<BINS;b++){double l=b*STEP,r=std::min(102.0,(b+1)*STEP);int mx=53;for(double q:f){double a=l+(r-l)*q;if(!(a>0))a=1e-15;if(a==3.0)a=std::nextafter(3.0,q<0.5?0.0:INFINITY);double rr=e.reference(a);mx=std::max(mx,e.pmin(a,rr));}L[b]=(uint16_t)mx;if((b&127)==0)std::fprintf(stderr,"[lut] %d/%d baseP=%u\n",b,BINS,(unsigned)L[b]);}return L;}
static void validate(Env&e,const std::vector<uint16_t>&L,int guard,const std::vector<double>&A,const std::vector<double>&R,const std::vector<int>&PM){uint64_t mxu=0;int gt1=0,worst=0,maxOver=-999,minOver=999,minP=999,maxP=0;long long sumP=0,sumM=0,sumO=0;for(size_t i=0;i<A.size();i++){int ps=std::min(400,(int)L[bin_of(A[i])]+guard);uint64_t u=ulp(e.eval(A[i],ps),R[i]);if(u>mxu){mxu=u;worst=(int)i;}gt1+=u>1;int ov=ps-PM[i];sumP+=ps;sumM+=PM[i];sumO+=ov;maxOver=std::max(maxOver,ov);minOver=std::min(minOver,ov);minP=std::min(minP,ps);maxP=std::max(maxP,ps);}std::printf("LUT step=1/16 bins=%d bytes=%d guard=%d GT1=%d/%zu MAX_ULP=%llu PMIN_MEAN=%.4f P_MEAN=%.4f MEAN_OVER=%.4f MIN_OVER=%d MAX_OVER=%d MIN_P=%d MAX_P=%d WORST_A=%.17g PMIN=%d PSCHED=%d\n",BINS,BINS*2,guard,gt1,A.size(),(unsigned long long)mxu,(double)sumM/A.size(),(double)sumP/A.size(),(double)sumO/A.size(),minOver,maxOver,minP,maxP,A[worst],PM[worst],std::min(400,(int)L[bin_of(A[worst])]+guard));}

int main(){Env e;constexpr int N=5000;std::vector<double>A(N),R(N);std::vector<int>PM(N);std::mt19937_64 g1(0x504d494e5f494e31ULL),g2(0x504d494e5f4f5554ULL);std::uniform_real_distribution<double>u1(1e-12,3.0),u2(3.0,102.0);for(int i=0;i<N/2;i++){double a=u1(g1);if(a>=3)a=std::nextafter(3.0,0.0);A[i]=a;}for(int i=N/2;i<N;i++){double a=u2(g2);if(a<=3)a=std::nextafter(3.0,INFINITY);A[i]=a;}const double hard[]={1e-12,1e-9,1e-6,0.01,0.1,1,2,2.9,2.99,2.9999,std::nextafter(3.0,0.0),std::nextafter(3.0,INFINITY),3.0001,3.01,4,10,20,40,50,75,81.764554025359956,100,101,102};for(size_t i=0;i<sizeof(hard)/sizeof(hard[0]);i++)A[i]=hard[i];int mn=999,mx=0;long long sm=0;for(int i=0;i<N;i++){R[i]=e.reference(A[i]);PM[i]=e.pmin(A[i],R[i]);mn=std::min(mn,PM[i]);mx=std::max(mx,PM[i]);sm+=PM[i];if((i&255)==0)std::fprintf(stderr,"[oracle] %d/%d a=%.9g Pmin=%d T=%d\n",i,N,A[i],PM[i],terms(A[i]));}std::printf("ORACLE N=%d PMIN_MIN=%d PMIN_MEAN=%.4f PMIN_MAX=%d\n",N,mn,(double)sm/N,mx);auto L=build_lut(e);for(int g=0;g<=4;g++)validate(e,L,g,A,R,PM);FILE*f=std::fopen("precision_lut_base_1_16.txt","w");if(f){std::fprintf(f,"// base per-input precision LUT; a-bin width=1/16; add tested guard at runtime\n");for(int b=0;b<BINS;b++)std::fprintf(f,"%u%c",(unsigned)L[b],((b+1)%32)?' ':'\n');std::fclose(f);}flint_cleanup();return 0;}
