...
int main(int argc,char**argv){
    if(argc!=2){std::fprintf(stderr,"usage: %s verify|raw|custom2|xeon\n",argv[0]);return 2;}
    std::string mode=argv[1];
    if(mode!="verify"&&mode!="raw"&&mode!="custom2"&&mode!="xeon") return 2;
    ...
    if(mode=="verify"){
        ...
        for(std::size_t n:sizes){
            zeta53_local9_batch(x.data(),n,a.data()); custom.run(x.data(),n,b.data());
            std::size_t diff=0; for(std::size_t i=0;i<n;++i) diff += std::memcmp(&a[i],&b[i],sizeof(double))!=0;
            total+=diff; std::printf("VERIFY BATCH=%zu BITDIFF=%zu\n",n,diff);
        }
