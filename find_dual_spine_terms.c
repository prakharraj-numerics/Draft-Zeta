...
int main(void){
  ...
  for(int i=0;i<N;i++){
    ...
    arb_set_d(ss,ain[i]-2.0); arb_zeta(ref,ss,PREC); rin[i]=arf_get_d(arb_midref(ref),ARF_RND_NEAR);
    arb_set_d(ss,aout[i]-2.0); arb_zeta(ref,ss,PREC); rout[i]=arf_get_d(arb_midref(ref),ARF_RND_NEAR);
  }
  int best1=0,best2=0;
  ...
  for(int T=1;T<=MAXT;T++){
    uint64_t mx=0; int bad=0; for(int i=0;i<N;i++){ double p=eval(thi,tlo,T,aout[i]); double y=1.0/(aout[i]-3.0)+1.0/3.0+p; uint64_t u=ulp(y,rout[i]); if(u>mx)mx=u; if(u>1)bad++; }
  ...
  if(best2){ uint64_t mx=0; int gt1=0; for(int i=0;i<N;i++){double y=1.0/(aout[i]-3.0)+1.0/3.0+eval(thi,tlo,best2,aout[i]);uint64_t u=ulp(y,rout[i]); if(u>mx)mx=u; gt1+=u>1;} printf("CASE2_VERIFY N=%d TERMS=%d MAX_ULP=%llu GT1=%d\n",N,best2,(unsigned long long)mx,gt1); }
  arb_clear(ref); arb_clear(ss); arb_clear(pow3); arb_clear(t); arb_clear(c); arb_clear(one); arb_poly_clear(z); arb_poly_clear(s); flint_cleanup(); return 0;
}
