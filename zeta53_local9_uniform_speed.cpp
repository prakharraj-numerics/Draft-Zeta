...
int main() {
    ...
    for (std::size_t n : sizes) {
        ...
        const double eps = 1.0e9 / mean;
        std::printf("ZETA_SPEED BATCH=%zu REPS=%d SAMPLES=11 MEAN_NS_PER_INPUT=%.6f MEDIAN_NS_PER_INPUT=%.6f EVALS_PER_SEC=%.3f\n",
                    n, reps, mean, median, eps);
    }
    std::printf("SINK=%.17g\n", (double)sink);
    return 0;
}
