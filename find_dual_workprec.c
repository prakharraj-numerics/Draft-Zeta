...
int main(){int W[]={512,1024,2048,4096};int nw=sizeof(W)/sizeof(W[0]);arb_poly_t s,z;arb_poly_init(s);arb_poly_init(z);arb_t one;arb_init(one);arb_one(one);arb_poly_set_coeff_si(s,0,-2);arb_poly_set_coeff_si(s,1,1);arb_poly_zeta_series(z,s,one,0,T1+1,GENPREC);
 ...
 for(int wi=0;wi<nw;wi++){slong wp=W[wi];int b1=0,b2=0;uint64_t mx1=0,mx2=0;arb_t a,p,y,tmp,third;arb_init(a);arb_init(p);arb_init(y);arb_init(tmp);arb_init(third);arb_one(third);arb_div_ui(third,third,3,wp);
  ...
  printf("WORKBITS=%d CASE1_GT1=%d MAX=%llu CASE2_GT1=%d MAX=%llu\n",W[wi],b1,(unsigned long long)mx1,b2,(unsigned long long)mx2);fflush(stdout);arb_clear(third);arb_clear(tmp);arb_clear(y);arb_clear(p);arb_clear(a);}
